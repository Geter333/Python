...
def load_data():
    # Завантаження даних
    input_file = '123.csv'  # Ім'я вхідного файлу
    try:
        # Читання CSV файлу
...
def plot_gdp_trends(df):
    ...
    gdp_ukr = df[df['Country Name'] == 'Poland'].iloc[0, 4:].astype(float)
    ...
    plt.plot(years, gdp_ukr, label='Польща', marker='o')

    plt.title('Динаміка GDP на душу населення (2004-2023)')
    plt.xlabel('Рік')
...
def plot_bar_chart(df, country_name):
    ...
    plt.title(f'GDP на душу населення для {country_name} (2004-2023)')
    plt.xlabel('Рік')
    plt.ylabel('GDP на душу населення (current US$)')
    plt.xticks(rotation=45)
    plt.grid(axis='y')
...
def main():
    ...
    if df is not None:
        ...
        while True:
            ...
            country_name = input(
                "Введіть назву країни для побудови стовпчастої діаграми (наприклад, 'Poland' або 'United States'), або введіть 'exit' для виходу: ")
